...
| `SlaveSim` | 自写 Modbus TCP 从站（设备模拟器，用 NModbus）。**多从站分区**：按 4 个区各开一个端口（综采三机 1502 / 运输 1503 / 供液 1504 / 供电 1505），模拟井下多控制器拓扑 |
...
## 布局（顶部连接栏 + 左侧 7 个 tab + 右侧常驻报文列）
- **右侧常驻「收发报文」列**（核心功能，不再是 tab）：上=报文表（发→/←收，写操作高亮、可只看写/暂停/清空），下=选中帧「逐字段解码」(MBAP+PDU 每字节)；中间可拖动调上下比例，左侧分隔条可调列宽。**SlaveSim 与 OpenPLC 两路连接的报文都汇到这里**，PLC 报文带 `[PLC]` 前缀区分。
- **左侧 7 个 tab**：

1. 顶部点「连接全部分区」（一键连 1502/1503/1504/1505 四个区从站，IP 默认 127.0.0.1）→ 上位机为每个区维护独立连接、分区轮询(800ms)；顶部 KPI 显示 设备/运行/报警 数；右侧报文按区带前缀（[综采三机]/[运输系统]/[供液系统]/[供电系统]/[PLC]）。
2. **总览大屏**：KPI 卡 + 实时趋势（可选监测点）+ 实时报警列表。
3. **设备控制**（工业 HMI 风格控制台）：每台设备一张卡——运行灯 + 关键读数 + **启停开关**、**设定值滑块+输入框+下发**、支架**升柱/降柱/移架**、组合开关**合闸/分闸**、支架群压力**一键 FC16 群写**。
4. **综采三机 / 运输系统 / 供液·供电**：分系统设备表（报警行红底）；供液供电页含「组合开关回路电流」「支架群压力」柱状图。
5. **PLC 控制**：独立连 OpenPLC(:502)，演示乳化泵启停联锁（PLC 否决上位机指令）。
6. **写入/调度**（题目二.3）：按位写(`位`=3,值=1，read-modify-write)、多字节(值`100,200`→FC16)、一键群写、次数+周期（任务表显示剩余）。

> 操作任意控制/写入时，右侧报文列实时滚动，点任一帧即可看逐字节解码——演示时"控制动作 ↔ 协议报文"一屏对照。
